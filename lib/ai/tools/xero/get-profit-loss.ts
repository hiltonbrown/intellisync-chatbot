...
import { getProfitLoss } from "@/lib/integrations/xero/operations";
import { getActiveTenantBinding } from "@/lib/db/queries";

export const getXeroProfitLoss = tool({
	description:
		"Get Profit & Loss (P&L) report from Xero accounting system. Shows income, expenses, and net profit/loss for a specified period.",
	...
	execute: async (input) => {
		try {
			...
			if (input.fromDate) options.fromDate = input.fromDate;
			if (input.toDate) options.toDate = input.toDate;
			if (input.periods) options.periods = input.periods;
			if (input.timeframe) options.timeframe = input.timeframe;

			// 5. Call Xero API
			const report = await getProfitLoss(client, options);
			...
			console.error("Failed to fetch Xero Profit & Loss:", error);
			...
			return { error: `Failed to retrieve Profit & Loss: ${errorMessage}` };
		}
	},
});
