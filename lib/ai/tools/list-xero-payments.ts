...
export const listXeroPayments = tool({
	description: "Lists payments from Xero.",
	inputSchema: z.object({
		page: z.number().int().min(1).optional(),
		fromDate: z.string().optional(),
		toDate: z.string().optional(),
	}),
	execute: async ({ page = 1, fromDate, toDate }) => {
		try {
			...
			return await withTokenRefreshRetry(binding.id, orgId, async (client) => {
				...
				const response = await client.fetch(`/Payments?${params.toString()}`);
			...
			return handleXeroToolError(error, { toolName: "listXeroPayments", operation: "fetching payments" });
		}
	},
});
