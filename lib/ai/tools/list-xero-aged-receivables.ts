...
export const listXeroAgedReceivables = tool({
	description: "Retrieves Aged Receivables by Contact report from Xero.",
	...
	execute: async ({ contactId, date, fromDate, toDate }) => {
		try {
			...
			return handleXeroToolError(error, { toolName: "listXeroAgedReceivables", operation: "fetching aged receivables" });
		}
	},
});
