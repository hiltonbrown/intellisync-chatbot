...
import { cashflowAdjustments, integrationTenantBindings, xeroTransactions } from "@/lib/db/schema";
...
export async function generateCashflowSuggestions() {
    ...
    const context = transactions.map(t => `${t.description}: $${t.amount} on ${t.date?.toISOString().split('T')[0]}`).join('\n');
...
export async function addCashflowAdjustment(data: { description: string; amount: number; date: Date; type: "IN" | "OUT" }) {
