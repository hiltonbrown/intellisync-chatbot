...
const PureSpreadsheetEditor = ({ content, saveContent }: SheetEditorProps) => {
	...
	const handleRowsChange = (newRows: any[]) => {
		...
		for (let i = updatedData.length - 1; i >= 0; i--) {
			if (updatedData[i].some((cell) => cell.trim() !== "")) {
				lastNonEmptyIndex = i;
				break;
			}
		}

		// Slice up to the last non-empty row (inclusive).
		// If all are empty (-1), slice(0, 0) returns [], which generates "" from unparse.
		const trimmedData = updatedData.slice(0, lastNonEmptyIndex + 1);

		const newCsvContent = generateCsv(trimmedData);
