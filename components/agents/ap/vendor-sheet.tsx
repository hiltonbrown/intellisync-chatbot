...
import { generateBillCommentary, getVendorDetails } from "@/lib/agents/ap/actions";
...
export function VendorSheet({
	...
	const handleGenerateCommentary = async (billId: string, summary: string, amount: string, due: string) => {
		if (!data) return;
		setLoadingCommentary(prev => ({ ...prev, [billId]: true }));
		try {
			const text = await generateBillCommentary(data.supplier.name, summary, amount, due);
							...
							<TabsContent value="unpaid">
								<ScrollArea className="h-[500px] pr-4">
									<div className="space-y-4">
										{data.bills.filter(b => Number(b.amountDue) > 0).map((bill) => (
											<div
												key={bill.id}
											...
											>
												<div className="flex justify-between items-start">
													<div>
														...
														<p className="text-xs text-muted-foreground">
															Due: {bill.dueDate ? new Date(bill.dueDate).toLocaleDateString("en-AU") : "N/A"}
														</p>
													</div>
													<div className="text-right">
														<p className="font-bold text-red-600">${bill.amountDue.toFixed(2)}</p>
														<p className="text-xs text-muted-foreground">Total: ${bill.total.toFixed(2)}</p>
													</div>
												</div>
												...
												<div className="bg-muted/50 p-2 rounded-md text-sm">
													{commentaries[bill.id] ? (
														<p className="italic text-muted-foreground">"{commentaries[bill.id]}"</p>
													) : (
														<div className="flex items-center justify-between">
															<span className="text-xs text-muted-foreground">No commentary generated.</span>
															<Button
																variant="ghost"
																size="sm"
																className="h-6"
																...
																onClick={() => handleGenerateCommentary(
																	...
																	`$${bill.amountDue}`,
																	bill.dueDate ? new Date(bill.dueDate).toLocaleDateString("en-AU") : "Unknown"
																)}
															>
																<Sparkles className="w-3 h-3 mr-1" />
																{loadingCommentary[bill.id] ? "Thinking..." : "Explain"}
															</Button>
														</div>
													)}
												</div>
                                        ...
                                        {data.bills.filter(b => Number(b.amountDue) > 0).length === 0 && (
                                            <div className="text-center text-muted-foreground py-8">No unpaid bills.</div>
                                        )}
									</div>
								</ScrollArea>
							</TabsContent>
										...
										{data.bills.filter(b => Number(b.amountDue) === 0).map((bill) => (
											...
											>
												<div>
													...
													<p className="text-xs text-muted-foreground">
														Paid Date: {bill.updatedAt ? new Date(bill.updatedAt).toLocaleDateString("en-AU") : "N/A"}
													</p>
												</div>
												<div className="text-right">
