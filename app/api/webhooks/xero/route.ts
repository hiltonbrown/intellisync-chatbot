...
import { ConfigError, logError, WebhookError } from "@/lib/integrations/errors";
...
export async function POST(req: Request) {
	...
	try {
		...
		if (
			contentLength &&
			Number.parseInt(contentLength, 10) > MAX_WEBHOOK_SIZE
		) {
		...
		for (const event of events) {
			...
			try {
				...
				if (
					...
					e !== null &&
					"code" in e &&
					e.code === POSTGRES_UNIQUE_VIOLATION
				) {
